...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      localStorage.setItem(`profile_${data.user.id}`, JSON.stringify({ id: data.user.id, name, email, role, language: "", grade: 0, subjects: [], streak: 0 }));
  ...
  const handleGoogleSignup = async () => {
    await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: window.location.origin + "/onboard" } });
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-background">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link to="/" className="font-heading text-2xl font-bold gold-text">🇮🇳 MaatruBhasha</Link>
          <p className="text-muted-foreground mt-2">Create your account and start learning today.</p>
        </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-3 mt-2">
                ...
                <Button type="button" variant={role === "teacher" ? "default" : "outline"} className="flex-1" onClick={() => setRole("teacher")}>
                  👨‍🏫 Teacher / Parent
                </Button>
              </div>
            </div>
